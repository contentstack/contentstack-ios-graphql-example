...
class ContentstackService {
    ...
    init(credentials: ContentstackCredentials = ContentstackCredentials.default) {
        let url = URL(string: "https://\(credentials.domainHost)/stacks/\(credentials.apiKey)?environment=\(credentials.environmentToken)&access_token=\(credentials.deliveryAPIAccessToken)")!
